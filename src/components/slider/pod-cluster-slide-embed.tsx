...
/** Single pod table for slide embeds (Floor Plans band). */
export function PodClusterSlideEmbed() {
  const reducedMotion = useReducedMotion();
  const pod = useMemo(() => {
    const { pods } = computeFindSpaceFloorPlan(FIND_SPACE_DEFAULT_FILTER_VALUES);
    return pods.find((entry) => entry.id === DISPLAY_POD_IDS[0] && entry.visible) ?? null;
  }, []);
