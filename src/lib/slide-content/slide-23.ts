import type { SlideDefinition } from "@/types/slide-content";

export const slide23: SlideDefinition = {
  id: "slide-23",
  label: "Inbox",
  backgroundColor: "#F2F0F6",
  align: "center",
  blockGap: "gap-4",
  className: "text-black !items-start justify-center pt-10",
  blocks: [
    {
      id: "slide-23-heading",
      type: "title",
      text: "Notification",
      textCase: "preserve",
      fontSize: 20,
      className: "index-slide-about-body !font-bold !leading-snug",
    },
    {
      id: "slide-23-body",
      type: "body",
      text: "A new unified inbox groups actionable system messages together and no longer clogs up the user's email. Push notifications on mobile provide lightweight, actionable information",
      textCase: "preserve",
      fontSize: 17,
      className: "index-slide-about-body max-w-3xl !leading-snug",
    },
    {
      id: "slide-23-media",
      type: "image",
      placeholder: true,
      placeholderVariant: "slide-23",
      alt: "Inbox with notification list and update details panel",
      align: "center",
      className:
        "mx-auto h-[530px] w-[880px] max-w-full overflow-hidden rounded-[20px] border border-black/10 bg-white shadow-[0_8px_32px_rgba(28,37,46,0.08)]",
    },
  ],
};
